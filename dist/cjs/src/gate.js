...
class gate extends gate$1 {
  ...
  spotOrderPrepareRequest(market = undefined, stop = false, params = {}) {
    ...
    if (!stop) {
      if (market === undefined) {
        throw new errors.ArgumentsRequired(
          this.id + ' spotOrderPrepareRequest() requires a market argument for non-stop orders'
        )
      }
      ...
      if (addressField !== undefined) {
        if (addressField.indexOf('New address is being generated for you, please wait') >= 0) {
          throw new errors.BadResponse(
            this.id +
              ' ' +
              'New address is being generated for you, please wait a few seconds and try again to get the address.'
          )
        }
  ...
  async fetchTickers(symbols = undefined, params = {}) {
    ...
    if (type === 'spot' || type === 'margin') {
      ...
      throw new errors.NotSupported(
        this.id +
          ' fetchTickers() not support this market type, provide symbols or set params["defaultType"] to one from spot/margin/swap/future/option'
      )
    }
  ...
  createOrdersRequest(orders, params = {}) {
    ...
    for (let i = 0; i < orders.length; i++) {
      ...
      const triggerValue = this.safeValueN(orderParams, [
        'triggerPrice',
        'stopPrice',
        'takeProfitPrice',
        'stopLossPrice'
      ...
      if (triggerValue !== undefined) {
        throw new errors.NotSupported(
          this.id +
            ' createOrders() does not support advanced order properties (stopPrice, takeProfitPrice, stopLossPrice)'
        )
      }
  ...
  createOrderRequest(symbol, type, side, amount, price = undefined, params = {}) {
    ...
    if (isStopLossOrder && isTakeProfitOrder) {
      throw new errors.ExchangeError(
        this.id + ' createOrder() stopLossPrice and takeProfitPrice cannot both be defined'
      )
    }
    ...
    if (isMarketOrder) {
      if (timeInForce === 'poc' || timeInForce === 'gtc') {
        throw new errors.ExchangeError(
          this.id + ' createOrder () timeInForce for market order can only be "FOK" or "IOC"'
        )
      } else {
    ...
    if (nonTriggerOrder) {
      if (contract) {
        ...
        if (isMarketOrder && side === 'buy') {
          ...
          ;[createMarketBuyOrderRequiresPrice, params] = this.handleOptionAndParams(
            params,
            'createOrder',
      ...
      if (clientOrderId !== undefined) {
        ...
        if (clientOrderId.length > 28) {
          throw new errors.BadRequest(
            this.id + ' createOrder () clientOrderId or text param must be up to 28 characters'
          )
        }
      ...
      if (contract) {
        ...
        if (trigger === undefined) {
          ...
          if (priceType < 0 || priceType > 2) {
            throw new errors.BadRequest(
              this.id + ' createOrder () price_type should be 0 latest deal price, 1 mark price, 2 index price'
            )
          }
  ...
  editOrderRequest(id, symbol, type, side, amount = undefined, price = undefined, params = {}) {
    ...
    if (account === 'spot') {
      if (!isLimitOrder) {
        ...
        throw new errors.InvalidOrder(
          this.id + ' editOrder() does not support ' + type + ' orders for ' + marketType + ' markets'
        )
      }
    }
    const request = {
  ...
  fetchOrderRequest(id, symbol = undefined, params = {}) {
    ...
    const contract = type === 'swap' || type === 'future' || type === 'option'
    const [request, requestParams] = contract
      ? this.prepareRequest(market, type, query)
      : this.spotOrderPrepareRequest(market, stop, query)
  ...
  fetchOrdersByStatusRequest(status, symbol = undefined, since = undefined, limit = undefined, params = {}) {
    ...
    const [request, requestParams] = spot
      ? this.multiOrderSpotPrepareRequest(market, stop, query)
      : this.prepareRequest(market, type, query)
    if (status === 'closed') {
      status = 'finished'
    }
  ...
  async transfer(code, amount, fromAccount, toAccount, params = {}) {
    ...
    if (fromId === 'margin' || toId === 'margin') {
      ...
      if (symbol === undefined) {
        throw new errors.ArgumentsRequired(
          this.id + ' transfer requires params["symbol"] for isolated margin transfers'
        )
      }
      const market = this.market(symbol)
      request['currency_pair'] = market['id']
  ...
  sign(path, api = [], method = 'GET', params = {}, headers = undefined, body = undefined) {
    ...
    if (containsSettle && path.endsWith('batch_cancel_orders')) {
      // weird check to prevent $settle in php and converting {settle} to array(settle)
  ...
  async fetchOpenInterestHistory(symbol, timeframe = '5m', since = undefined, limit = undefined, params = {}) {
    ...
    if (paginate) {
      return await this.fetchPaginatedCallDeterministic(
